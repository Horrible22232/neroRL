...
def wrap_environment(config, worker_id, realtime_mode = False, record_trajectory = False):
    ...
    if config["type"] == "Unity":
        env = UnityWrapper(config["name"], worker_id, realtime_mode=realtime_mode, record_trajectory=record_trajectory)
        ...
        env = MinigridWrapper(config["name"], realtime_mode=realtime_mode, record_trajectory=record_trajectory)
    elif config["type"] == "Procgen":
        env = ProcgenWrapper(config["name"], realtime_mode=realtime_mode, record_trajectory=record_trajectory)
    elif config["type"] == "CartPole":
        env = CartPoleWrapper(config["name"], realtime_mode=realtime_mode, record_trajectory=record_trajectory)
